...
$$
\hat{H} = \frac{\eta}{2} \sum_{j=1}^2 \hat{n}_j (\hat{n}_j - 1) + g(t) \left( \hat{a}_2^\dagger \hat{a}_1 + \hat{a}_1^\dagger \hat{a}_2 \right) + \sum_{j=1}^2 \delta_j(t) \hat{n}_j + \sum_{j=1}^2 i f_j(t) \left( \hat{a}_j e^{-i \varphi_j(t)} - \hat{a}_j^\dagger e^{i \varphi_j(t)} \right)
$$
